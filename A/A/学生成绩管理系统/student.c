...
void in_comput()//输入基本信息，计算每个学生各门功课总分sum、平均分aver。
{
	...
	while (1)
	{
		scanf_s("%d %s %c", &stu[N].num, stu[N].name, sizeof(stu[N].name), &stu[N].sex, sizeof(stu[N].sex));
		for (i = 0; i < 5; i++)
		{
			scanf_s(" %lf", &stu[N].score[i]);
			...
			if (s == 1)
			{
				...
				for (i = 0; i < N; i++)
					if (strcmp(name, stu[i].name) == 0)
					{
						...
						scanf_s("%d %s %c %lf %lf %lf %lf %lf", &stu[i].num, stu[i].name, sizeof(stu[i].name), &stu[i].sex, sizeof(stu[i].sex), &stu[i].score[0], &stu[i].score[1], &stu[i].score[2], &stu[i].score[3], &stu[i].score[4]);
					}
				if (t == 0)
					printf("\a该生不存在\n");
			}
...
void Read()//将学生信息从文件读出
{
	...
	errno_t err;
	int i;
	if ((err = fopen_s(&fp, "学生成绩管理系统\\Student Database.txt", "rb")) != 0)
		return;
	if (fread(&N, sizeof(int), 1, fp) != 1)//if (fscanf_s(fp, "%d", &N) < 0)//
		N = -1;
	else
		for (i = 0; i<N; i++)
			fread(&stu[i], sizeof(struct Student), 1, fp);//fscanf_s(fp,"%d %s %c %.2f %.2f %.2f %.2f %.2f %.2f %.2f", &stu[i].num, stu[i].name, &stu[i].sex, &stu[i].score[0], &stu[i].score[1], &stu[i].score[2], &stu[i].score[3], &stu[i].score[4], &stu[i].sum, &stu[i].aver);//
	fclose(fp);
}
void Write()//将学生信息写入文件
{
	...
	errno_t err;
	int i;
	if ((err = fopen_s(&fp, "学生成绩管理系统\\Student Database.txt", "wb")) != 0)
	{
		printf("\a不能打开文件\n");
		return;
	}
